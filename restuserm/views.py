...
def callback42(request):
    ...
    if request.user.is_authenticated:
        return Response({"message" : "Already signed in", "statusCode" : 200,})
    if code:
        ...
        if response.status_code == 200:
            ...
            if not user_response.ok:
                return Response({'detail': 'No access token in the token response'}, status=status.HTTP_400_BAD_REQUEST)
            user_data = {
...
def tfa_verification(request):
    ...
    if not tfa:
        ...
        player.save()
        return Response({'message' : 'The player has successfully verified', 'statusCode' : 200})
    else:
        if not tfa_code_check(id_player, code):
            return Response({'error' : '2FA code is incorrect', 'statusCode' : 401})
        token_jwt = jwt_generation(id_player, False)
        response = Response({'message' : 'The player has successfully verified', 'statusCode' : 200, 'redirected' : True})
