...
def saveData(self):
    addUserToDB = pongGameInfo(
        login=self.scope["user"], codeToPlay=roomcode(self.scope["user"])
    )
    addUserToDB.save()
        ...
        try:
            ...
            if dataFromClient.get("gameStatus") == "onprogress":
                ...
                if dataFromClient.get("move") == "UP":
                    if (
                        self.playersOnMatchAndItsRoomId.get(thisUser)
                        == pongGameInfo.objects.get(login=thisUser).codeToPlay
                    ):
                        paddle1 -= 20
                    else:
                        paddle2 -= 20
                ...
                if self.playersOnMatchAndItsOppenent.get(thisUser) is not None:
                    ...
                    roomidForThisUser = self.playersOnMatchAndItsRoomId.get(thisUser)
                    leftedGame, Win = pongGameInfo.objects.get(
                        login=thisUser
                    ), pongGameInfo.objects.get(login=oppenent)
                    leftedGame.loses += 1
                    leftedGame.save()
                    Win.wins += 1
                    Win.save()
                    print(
                        f"Add Historic Of The Match Between {thisUser} and {oppenent}"
                    )
                    user1 = pongHistory(
                        you=thisUser, oppenent=oppenent, winner=oppenent
                    )
                    user2 = pongHistory(
                        you=oppenent, oppenent=thisUser, winner=oppenent
                    )
                    user1.save()
                    user2.save()
                    ...
                    toFront = {
                        "MoveFor": "end",
                        "winner": Win.login,
                        "loser": leftedGame.login,
                        "winnerPic": playerAndHisPic.objects.get(login=Win.login).pic,
                        "loserPic": playerAndHisPic.objects.get(
                            login=leftedGame.login
                        ).pic,
                    }
                    print(f"=> {toFront}")
                    await self.channel_layer.group_send(
                        roomidForThisUser, {"type": "endGame", "Data": toFront}
                    )
                else:
                    try:
                ...
                print("ENDING....")
                if (
                    self.playersOnMatchAndItsDeriction.get(thisUser) == "Left"
                    and dataFromClient.get("Side") == "LEFT"
                ):
                    loser = thisUser
                    winner = oppenent
                elif (
                    self.playersOnMatchAndItsDeriction.get(thisUser) == "Right"
                    and dataFromClient.get("Side") == "RIGHT"
                ):
                    loser = thisUser
                    winner = oppenent
                else:
                ...
                try:
                    ...
                    print(f"=> {toFront}")
                    await self.channel_layer.group_send(
                        roomId, {"type": "endGame", "Data": toFront}
                    )
                except:
    ...
    async def disconnect(self, code):
        ...
        try:
            ...
            if roomidForThisUser is not None:
                await self.channel_layer.group_discard(
                    roomidForThisUser, self.channel_name
                )
...
class myPongserver(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if len(self.playerWantsToPlay) == 0:
            ...
            if self.playersOnMatchAndItsOppenent.get(player1) is not None:
                ...
                await self.channel_layer.group_send(
                    roomid, {"type": "ToFrontOnConnect", "Data": toFronEnd}
                )
        else:
            ...
            if player1 == player2:
                ...
                roomid = pongGameInfo.objects.get(
                    login=self.playerWantsToPlay[0]
                ).codeToPlay
                ...
                self.playerAndHisPaddle[player1] = "paddle1"
                self.playerAndHisPaddle[player2] = "paddle2"
                await self.channel_layer.group_add(roomid, self.channel_name)
                ...
                print(f"Player1: {player1}, Player2: {player2}, RoomId: {roomid}")
                await self.channel_layer.group_send(
                    roomid, {"type": "ToFrontOnConnect", "Data": toFrontEnd}
                )
    ...
    async def receive_json(self, dataFromClient, bytes_data=None):
        try:
            # print(dataFromClient)
            thisUser, oppenent = self.scope[
                "user"
            ], self.playersOnMatchAndItsOppenent.get(self.scope["user"])
            roomidForThisUser = self.playersOnMatchAndItsRoomId.get(thisUser)
            if dataFromClient.get("gameStatus") == "onprogress":
                BallRoute, BallDirection = dataFromClient.get(
                    "BallRoute"
                ), dataFromClient.get("BallDir")
                ballx, bally = dataFromClient.get("ballx"), dataFromClient.get("bally")
                paddle1, paddle2 = dataFromClient.get("paddle1"), dataFromClient.get(
                    "paddle2"
                )
                if dataFromClient.get("move") == "UP":
                    if self.playerAndHisPaddle.get(thisUser) == "paddle1":
                        paddle1 -= 20
                    else:
                        paddle2 -= 20
                elif dataFromClient.get("move") == "DOWN":
                ...
                await self.channel_layer.group_send(
                    roomidForThisUser, {"type": "ToFrontOnConnect", "Data": toFront}
                )
            elif dataFromClient.get("gameStatus") == "closed":
                print(f"{self.scope['user']} Left")
                if self.playersOnMatchAndItsOppenent.get(thisUser) is not None:
    ...
    async def disconnect(self, code):
        ...
        try:
            ...
            try:
                ...
                print(f"{self.scope['user']} Play And Finish Already")
            destroyThisGameInformations(
                self.playersOnMatchAndItsOppenent,
                self.playersOnMatchAndItsRoomId,
                self.playersOnMatchAndItsDeriction,
...
class TournamentConsumer(AsyncWebsocketConsumer):
                    ...
                    text_data=json.dumps(
                        {
                            ...
                            "semi_final_results": [
                                {
                                    "match": self.semi_final_matchups[i],
                                    "winner": self.semi_final_winners[i],
                                }
                                for i in range(len(self.semi_final_matchups))
                            ],
                            "third_place_result": {
                                "match": self.final_matchups[0],
                                "winner": third_place_winner,
                            },
                            "final_result": {
                                "match": self.final_matchups[1],
                                "winner": tournament_winner,
                            },
                        }
                    )
    ...
    def end_match(self, winner):
        ...
        loser = (
            current_matchup[0] if winner != current_matchup[0] else current_matchup[1]
        )
